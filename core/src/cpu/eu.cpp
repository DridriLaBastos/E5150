...
static unsigned int prepareInstructionExecution(void)
{
	...
	switch(cpu.eu.mDecoder.decodedInstruction.name)
	{
		case DECODER::INSTRUCTION_NAME::MOV:
			instructionFunction = MOV;
			return getMOVCycles();

		case DECODER::INSTRUCTION_NAME::PUSH:
			instructionFunction = PUSH;
			return getPUSHCycles();

		case DECODER::INSTRUCTION_NAME::POP:
			instructionFunction = POP;
			return getPOPCycles();

		case DECODER::INSTRUCTION_NAME::XCHG:
			instructionFunction = XCHG;
			return getXCHGCycles();

		case DECODER::INSTRUCTION_NAME::IN:
			instructionFunction = IN;
			return getINCycles();

		case DECODER::INSTRUCTION_NAME::OUT:
			instructionFunction = OUT;
			return getOUTCycles();

		case DECODER::INSTRUCTION_NAME::XLAT:
...
static bool EUDecodeClock(void)
{
	...
	const bool instructionDecoded = cpu.eu.mDecoder.feed(cpu.biu.instructionBufferQueue[cpu.biu.instructionBufferQueuePos]);
