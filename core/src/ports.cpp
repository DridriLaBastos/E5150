...
uint8_t PORTS::read (const uint16_t portAddress) const 
{
	...
	if (found != m_portDevices.end())
	{
		...
		PORTSEmulationLog(EMULATION_MAX_LOG_LEVEL,"R ({}): {:#x} <-- {:#x} (local: {:#b})",portInfo.component->m_name,(unsigned)data,portAddress,componentLocalAddress);
	}
	else
		PORTSEmulationLog(8,"No device found at port address {:#x}. Result is watever is on the data bus ({:#x})",portAddress,data);
...
void PORTS::write(const uint16_t portAddress, const uint8_t data) 
{
	...
	if (found != m_portDevices.end())
	{
		...
		const unsigned int componentLocalAddress = portAddress & portInfo.addressMask;
			...
			PORTSEmulationLog(EMULATION_MAX_LOG_LEVEL,"W ({}): {:#x} --> {:#x} (local: {:#b})",portInfo.component->m_name,(unsigned)data,portAddress,componentLocalAddress);
		#endif
	}
	else
		PORTSEmulationLog(8,"No device found at port address {:#x}",portAddress);
}
