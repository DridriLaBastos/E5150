...
typedef enum {
	...
	DECOM_CONFIGURE_EMULATOR_TO_DEBUGGER,
	DECOM_CONFIGURE_DEBUGGER_TO_EMULATOR,
}DECOM_CONFIGURE;
...
extern "C" {
	...
	DLL_EXPORT DECOM_STATUS decom_ReadFromChannel(const DECOM_CONFIGURE channelConfigure, void* readBuffer, const size_t readBufferSizeInOctets, size_t* const ignoredIfNull_ReadCount);
	DLL_EXPORT DECOM_STATUS decom_WriteToChannel(const DECOM_CONFIGURE channelConfigure, void* writeBuffer, const size_t writeBufferSizeInOctets, size_t* const ignoredIfNull_WriteCount);
...
#define READ_FROM_EMULATOR(readBufferPtr, readBufferSizeInOctets) decom_ReadFromChannel(DECOM_CONFIGURE_DEBUGGER_TO_EMULATOR,readBufferPtr,readBufferSizeInOctets,NULL)
#define WRITE_TO_EMULATOR(writeBufferPtr, writeBufferSizeInOctets) decom_WriteToChannel(DECOM_CONFIGURE_EMULATOR_TO_DEBUGGER,writeBufferPtr,writeBufferSizeInOctets,NULL)

#define READ_FROM_DEBUGGER(readBufferPtr, readBufferSizeInOctets) decom_ReadFromChannel(DECOM_CONFIGURE_DEBUGGER_TO_EMULATOR,readBufferPtr,readBufferSizeInOctets,NULL)
#define WRITE_TO_DEBUGGER(writeBufferPtr, writeBufferSizeInOctets) decom_WriteToChannel(DECOM_CONFIGURE_EMULATOR_TO_DEBUGGER,writeBufferPtr,writeBufferSizeInOctets,NULL)
